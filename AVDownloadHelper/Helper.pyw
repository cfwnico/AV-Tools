...
# ToDo:
# 1.选择本地数据库后，可以管理数据库
# 2.qb推送下载时先检测是否已经存在于数据库中，如果存在则进行提示
# 3.可视化管理本地的av，显示名称、演员、等，并且可以进行更多的操作，例如删除。。导出excel表格，根据标签、演员进行筛选等等。。。
import json
import os
import sys
...
class MainWin(QMainWindow, MainWindow):
    ...
    def __init__(self, config: Config):
        ...
        self.config = config
        self.setupUi(self)
        self.tray_ico = QPixmap(os.path.join(self.work_dir, r"tray.ico"))
        self.msg_ico = QPixmap(os.path.join(self.work_dir, r"msg.ico"))
    ...
    def setup_tray(self):
        ...
        self.trayicon.setIcon(self.tray_ico)
